    ...
    public static void onServerTick(TickEvent.ServerTickEvent event) {
        ...
        for (ServerPlayer player : event.getServer().getPlayerList().getPlayers()) {
            player.getCapability(ZeroGCapability.ZERO_G_STATE).ifPresent(state -> {
                ...
                // 处理翻滚输入
                if (state.inputRollLeft) state.orientation.mul(new Quaternionf().rotateZ(-ROLL_SPEED));
                if (state.inputRollRight) state.orientation.mul(new Quaternionf().rotateZ(ROLL_SPEED));
                state.orientation.normalize();

                // 提取yaw/pitch同步给客户端（用于模型朝向）
                ...
                // 计算移动方向
                Vector3f f3 = new Vector3f(0,0,1), u3 = new Vector3f(0,1,0), r3 = new Vector3f(1,0,0);
                state.orientation.transform(f3); state.orientation.transform(u3); state.orientation.transform(r3);
                ...
                if (state.velocity.length() > MAX_SPEED) state.velocity = state.velocity.normalize().scale(MAX_SPEED);
                if (state.velocity.length() < 0.001) state.velocity = Vec3.ZERO;

                // 应用速度
                player.setDeltaMovement(state.velocity);
                player.setNoGravity(true);
                player.hurtMarked = true; // 强制同步位置
            });
        }
    }
